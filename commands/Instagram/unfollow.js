exports.run = (bot, message, suffix, help) => {
    ...
    request.get("www.instagram.com", `/web/search/topsearch/?context=blended&query=${suffix}&rank_token=0.8730298813193997&include_reel=false`, config.instagram.sessionId, config.instagram.userAgents.browser).then(body => {
        ...
        if (!result || result.length < 1)
            ...
            request.post(`/web/friendships/${result[0].user.pk}/unfollow/`, {}, config.instagram.sessionId).then(body => {
